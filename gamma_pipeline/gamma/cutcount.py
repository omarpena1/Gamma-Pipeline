...
def resample_counts_to_grid(
    x_target_keV: np.ndarray,
    *,
    x_src_keV: np.ndarray,
    y_src_counts: np.ndarray,
) -> np.ndarray:
    """
    Evaluate y_src (counts per bin at x_src bin centers) on a different grid x_target
    using 1D linear interpolation.

    Values outside the source energy coverage are set to 0.

    Note: This is a *bin-center* interpolation, not a proper rebin-by-bin-edges.
    For your current use (template subtraction + window integrals), this is usually
    sufficient as long as binning is reasonably fine.
    """
    x_t = np.asarray(x_target_keV, dtype=float).reshape(-1)
    x_s = np.asarray(x_src_keV, dtype=float).reshape(-1)
    y_s = np.asarray(y_src_counts, dtype=float).reshape(-1)
    if x_s.shape != y_s.shape:
        raise ValueError(f"x_src_keV and y_src_counts must have same shape. Got {x_s.shape} vs {y_s.shape}.")
    # Ensure monotonic x for np.interp
    if x_s.size >= 2 and np.any(np.diff(x_s) < 0):
        order = np.argsort(x_s)
        x_s = x_s[order]
        y_s = y_s[order]
    return np.interp(x_t, x_s, y_s, left=0.0, right=0.0)


def subtract_scaled_template_on_grid(
    *,
    x_mix_keV: np.ndarray,
    y_mix: np.ndarray,
    x_template_keV: np.ndarray,
    y_template: np.ndarray,
    scale: float,
) -> tuple[np.ndarray, np.ndarray]:
    """
    Subtract (scale * template) from mixture after evaluating template onto the mixture grid.

    Returns
    -------
    y_residual : np.ndarray
        Clipped residual counts on the mixture grid.
    y_template_on_mix : np.ndarray
        The template counts evaluated on the mixture grid (before scaling).
    """
    x_mix = np.asarray(x_mix_keV, dtype=float).reshape(-1)
    y_mix = np.asarray(y_mix, dtype=float).reshape(-1)
    if x_mix.shape != y_mix.shape:
        raise ValueError(f"x_mix_keV and y_mix must have same shape. Got {x_mix.shape} vs {y_mix.shape}.")

    y_tpl_on_mix = resample_counts_to_grid(x_mix, x_src_keV=x_template_keV, y_src_counts=y_template)
    y_res = np.clip(y_mix - float(scale) * y_tpl_on_mix, 0.0, None)
    return y_res, y_tpl_on_mix
...
def estimate_single_isotope_cutcount(
    *,
    ...
    x_template_keV: Optional[np.ndarray] = None,
    ...
    if y_template is not None:
        # If template is on a different energy grid, evaluate it on the sample grid (x_keV).
        y_tpl_on_x = np.asarray(y_template, dtype=float).reshape(-1)
        if x_template_keV is not None:
            y_tpl_on_x = resample_counts_to_grid(
                np.asarray(x_keV, dtype=float).reshape(-1),
                x_src_keV=np.asarray(x_template_keV, dtype=float).reshape(-1),
                y_src_counts=y_tpl_on_x,
            )
        else:
            # Back-compat: require same shape if you only pass y_template.
            if np.asarray(x_keV).reshape(-1).shape != y_tpl_on_x.shape:
                raise ValueError(
                    "Template axis mismatch: pass x_template_keV when y_template is not already on the sample grid."
                )

        C_tpl = integrate_counts_in_bounds(x_keV, y_tpl_on_x, bounds)
...
class CeAcCutCountConfig:
    method_tag: str = "cutcount_ceac_v3"
...
def estimate_ceac_mixture_cutcount(
    *,
    ...
    x_ce_template_keV: Optional[np.ndarray] = None,
    ...
    x = np.asarray(x_keV, dtype=float).reshape(-1)
    y_mix = np.asarray(y_mix, dtype=float).reshape(-1)
    if x.shape != y_mix.shape:
        raise ValueError(f"x_keV and y_mix must have same shape. Got {x.shape} vs {y_mix.shape}.")

    # Ce template may live on a different energy axis; evaluate it on the mixture grid.
    y_tpl_src = np.asarray(y_ce_template, dtype=float).reshape(-1)
    if x_ce_template_keV is None:
        # Back-compat: if only y is provided, it must already be on x grid.
        if y_tpl_src.shape != x.shape:
            raise ValueError(
                "Template axis mismatch: pass x_ce_template_keV when y_ce_template is not already on the mixture grid."
            )
        y_tpl_on_mix = y_tpl_src
        x_tpl = x
    else:
        x_tpl = np.asarray(x_ce_template_keV, dtype=float).reshape(-1)
        y_tpl_on_mix = resample_counts_to_grid(x, x_src_keV=x_tpl, y_src_counts=y_tpl_src)
    ...
    C_ce_tpl = integrate_counts_in_bounds(x, y_tpl_on_mix, ce_bounds)
    scale_ce = (C_ce / C_ce_tpl) if C_ce_tpl > 0 else 0.0
    y_ce_scaled = scale_ce * y_tpl_on_mix

    # residual
    y_res = np.clip(y_mix - y_ce_scaled, 0.0, None)
    ...
    fit_arrays = {
        "x_keV": x,
        "y_mix": y_mix,
        "y_template_ce": y_tpl_on_mix,
        "y_scaled_template_ce": y_ce_scaled,
        "y_ce_scaled": y_ce_scaled,  # back-compat alias
        # "x_ce_template_keV" : x_ce_template_keV,
        "y_residual": y_res,
    }
